...
### Phase 0 — Confirm scope (ALWAYS ask first)

Before any recon or audit, **ask the user what to audit** and wait for their answer — never default to scanning the whole app unprompted. Use the AskUserQuestion tool with options like:

- **Whole app** — every application surface (the standard full audit).
- **A specific area** — a directory or feature the user names (e.g. `src/components/map`, the checkout flow, the properties page).
- **One category** — bugs, performance, accessibility, security, or maintainability only.
- **A specific concern** — a symptom to chase (e.g. "the list re-renders on every keystroke", "the map is slow on mobile").

Skip this question ONLY when the invocation already carries a scope — a category focus (`improve-react performance`), a path/area named in the request, or `plan <description>`. Otherwise ask first, then scope Phase 1 recon and Phase 2 audit to the answer (recon still reads enough of the stack to gate rules correctly; the audit and findings stay within the chosen scope).
...
| `execute <plan>`                                                                         | Dispatch the **`react-plan-executor`** subagent (Opus at low effort, in its own worktree) to implement the plan, then review its diff against React Doctor (`--scope changed`) and render a verdict. Fall back to a general-purpose agent if it isn't installed |
